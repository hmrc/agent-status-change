...
trait WSResponseMatchers {
  ...
  def haveStatus(status: Int): Matcher[WSResponse] =
    new Matcher[WSResponse] {
      override def apply(left: WSResponse): MatchResult = MatchResult(
        left.status == status,
      ...
      override def apply(left: WSResponse): MatchResult =
        Try(left.json) match {
          case Success(o: JsObject) =>
            matchers.foldLeft(MatchResult(true, "", ""))((a, b) =>
              if (a.matches)
                b(o)
              else
                a
          ...
          case Success(_) =>
            MatchResult(
              true,
              "",
              "Have valid JSON body"
            )
          case Failure(e) =>
            MatchResult(
              false,
              s"Could not parse.tolerantJson body because of $e",
              ""
            )
        }
    }
      ...
      override def apply(left: WSResponse): MatchResult =
        Try(left.json) match {
          ...
          case Success(x) =>
            MatchResult(
              false,
              s"JSON value should be an array but was $x",
              ""
            )
          case Failure(e) =>
            MatchResult(
