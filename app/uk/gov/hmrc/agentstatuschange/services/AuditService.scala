...
class AuditService @Inject()(val auditConnector: AuditConnector) {
  ...
  def sendAgentstatuschangeSomethingHappened(
    model: AgentStatusChangeModel,
    agentReference: Arn)(implicit hc: HeaderCarrier, request: Request[Any], ec: ExecutionContext): Unit =
    auditEvent(
      AgentstatuschangeEvent.AgentstatuschangeSomethingHappened,
      "agent-status-change-something-happened",
      Seq(
  ...
  private[services] def createEvent(event: AgentstatuschangeEvent, transactionName: String, details: (String, Any)*)(
    implicit hc: HeaderCarrier,
    request: Request[Any],
    ...
    DataEvent(auditSource = "agent-status-change", auditType = event.toString, tags = tags, detail = detail)
  }
