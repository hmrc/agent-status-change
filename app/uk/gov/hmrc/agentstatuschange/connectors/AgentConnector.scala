...
class AgentConnector @Inject() (
  ...
  def removeAgentInvitations(arn: Arn)(implicit
    ...
    http
      .delete(url"$agentClientAuthorisationBaseUrl/agent-client-authorisation/agent/${arn.value}/terminate")
  ...
  def removeAFIRelationship(arn: Arn)(implicit
    ...
    http
      .delete(url"$agentFiRelationshipBaseUrl/agent-fi-relationship/agent/${arn.value}/terminate")
      .execute[HttpResponse]
      .map { r =>
        r.status match {
          case 200 =>
  ...
  def removeAgentMapping(arn: Arn)(implicit
    ...
    http
      .delete(url"$agentMappingBaseUrl/agent-mapping/agent/${arn.value}/terminate")
      .execute[HttpResponse]
      .map { r =>
        r.status match {
          case 200 =>
  ...
  def removeAgentClientRelationships(arn: Arn)(implicit
    ...
    http
      .delete(url"$agentClientRelationshipsBaseUrl/agent-client-relationships/agent/${arn.value}/terminate")
      .execute[HttpResponse]
      .map { r =>
        r.status match {
          case 200 =>
